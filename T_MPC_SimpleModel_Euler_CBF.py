# NMPC (modelo Euler) + filtro de seguridad CBF orden 2.
# Igual que T_MPC_SimpleModel_Quat_CBF.py pero con x = [nx, ny, nz, psi, ul, um, un, w].
# El filtro es el mismo (barrera en marco cuerpo). Solo cambian los indices del estado
# y la rotacion que usa la camara simulada (Rz(psi) en vez del cuaternion).
...
from Functions_CBF import rot_euler, camera_depth_sim, obstacle_memory, create_cbf_qp, cbf_filter
from T_MPC_SimpleModel_Euler_external import create_ocp_solver_description, get_initial_state
...
def main():
    ...
    # OBSTACULOS (sobre la trayectoria deseada)
    r_obs = 0.4
    margin = 0.6
    d_safe = r_obs + margin
    t_obs = [9.0, 24.0, 40.0]
    ...
    alpha2 = 1.5
    rho = 1e4
    u_max = np.array([3.0, 3.0, 3.0, 2.0])
    u_min = -u_max
    qp, H = create_cbf_qp(n_obs, rho)

    # NMPC
    model, f = f_system_simple_model()
    ocp = create_ocp_solver_description(x[:, 0], N_prediction, t_prediction, 3)
    ...
    for k in range(0, n_steps):
        ...
        v = x[4:7, k]
        w = x[7, k]
        R = rot_euler(psi)                                # solo el simulador usa la orientacion
    ...
    fig6 = plot_cbf_xy(x, ref, obstacles, r_obs, d_safe)
    ...
    fig8 = plot_cbf_views(x, ref, obstacles, r_obs, d_safe)
    fig8.savefig(os.path.join(pwd, "8_side3d.jpg"), dpi=150)

    np.savez(os.path.join(pwd, "run_data.npz"), x=x, ref=ref, u_nmpc=u_nmpc, u_safe=u_safe,
